public class Board {
	...
	public Board(int playerNumber, String[] playerName, int startingMoney) {
        ...
        while(playerNumberInTheGame != 1){
        	for (int i = 0; i < playerNumber; i++) {
        		if(players[i].getMoney().getMoney()<=0){
        		}
        		else{
        			try {
        				Thread.sleep(3000);
        			...
        			if(players[i].IsPlayerInJail()){
        				squares[10].doAction(players[i], this);
        			}
        			else {
        				System.out.println("••••••••••••••••••••••••••••••••••••");
        				...
        				squares[currentLocation].doAction(players[i],this);
	...
	public int chanceAction(Player player, int nthSquare){
		squares[nthSquare].doAction(player, this);
		return 0;
	}
}